...
class FilmeTest extends \PHPUnit\Framework\TestCase
{
    ...
    // Testes para os métodos setExibicao e getExibicao da classe Filme.
    public function testSetExibicaoFilme()
    {
        $filme = new Filme(1, 'Spider Man', 'Ação', 'Filme de Super Herói', '10', '180 min');

        $this->assertFalse($filme->getExibicao()); // Verifica se a exibição do filme está inicialmente definida como false.

        $filme->setExibicao(); // Define a exibição do filme como true.

        $this->assertTrue($filme->getExibicao()); // Verifica se a exibição do filme agora é true.

        $filme->setExibicao(); // Define a exibição do filme como false.

        $this->assertFalse($filme->getExibicao()); // Verifica se a exibição do filme agora é false novamente.
    }
}
